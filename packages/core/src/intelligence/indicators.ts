...
import { calculateAtrSeries, calculateSmaSeries } from "../analysis/candleAnalysis.ts";
...
export function calculateMacd(
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    // Guard against undefined as well as null: the EMA series may be shorter
    // than `values`, and `x !== null` alone would let an undefined element
    // reach the subtraction and produce NaN in the MACD series.
